...
const IdCardTemplate = () => {
  ...
  const handleViewQRCode = async () => {
    if (!qrCodeUrl) {
      ...
      const qrCodeData = JSON.stringify({
        ...
        mohterNumber: userData?.motherNumber,
      });
  ...
  const downloadIdCard = async () => {
    ...
    context.fillText(
      "ইনস্টিটিউট অব কম্পিউটার সায়েন্স এন্ড টেকনোলজি, ছাত্রাবাস",
      canvas.width / 2,
      50
    );
    ...
    img.onload = async () => {
      ...
      context.fillText(
        `Institute: ${userData?.institute || "Institute Name"}`,
        400,
        190
      );
      context.fillText(
        `ID Number: ${userData?.studentId || "ID12345"}`,
        400,
        230
      );
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100 text-gray-800">
      ...
      <div
        className="perspective-1000 sm:m-4 m-2 w-full sm:w-[400px] md:w-[500px] h-[350px] relative"
        {...swipeHandlers}
      >
        <div
          className={`transform-style-3d w-full h-full absolute transition-transform ${
            viewQRCode ? "rotate-y-180" : "rotate-y-0"
          }`}
        >
          {/* Front Side (ID Card) */}
            ...
            <div className="grid grid-cols-2 gap-4 w-full">
              ...
              <div className="flex justify-center items-center bg-gray-200 h-full w-full p-2">
                <img
                  ...
                  src={
                    userData?.photo || "https://dummyimage.com/600x400/ccc/000"
                  }
                  alt="Profile"
                />
              </div>
